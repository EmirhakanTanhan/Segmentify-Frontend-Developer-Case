...
function initTabs (recData, recContent, itemPerView) {
    ...
    for (var i = 0; i < tabs.length; i++) {
        tabs[i].addEventListener('click', function (tab) {
            ...
            document.querySelector(`[data-tab-key-pair="${tab.target.dataset.tabKey}"]`).classList.add('tab-content-active');

            //Render cards and then initialize swiper
            renderRecContent(tab.target.dataset.tabKey, recData, recContent);
            swiper(itemPerView);
        })
    }
}

async function createRecComponent ({root, segmentifyLoad, recContent, recSkeleton, itemPerView}) {
    try {
        var recData = await segmentifyLoad();
        renderRecSkeleton(root, recData, recSkeleton);
        initTabs(recData, recContent, itemPerView);
    } catch (err) {
        console.log('Some error occured while rendering segmentify widget: ' + err);
...
function renderRecContent (target, recData, recContent) {
    var root = document.querySelector(`[data-tab-key-pair="${target}"]`);

    root.innerHTML = `
    <div class="swiper">
        <div class="swiper-wrapper">
        
        ${recData.recommendedProducts[target].map(function (product) {
        return recContent(product);
    }).join('')}
        </div>
        <div class="swiper-button-prev"></div>
...
function renderRecSkeleton (root, recData, recSkeleton) {
    if (recSkeleton(recData)) root.innerHTML = recSkeleton(recData);
    else throw 'recSkeleton: Not enough categories';
}

function swiper (itemPerView) {
    new Swiper(".swiper", {
        slidesPerView: itemPerView - 2 > 2 ? itemPerView - 2 : 2,
        spaceBetween: 5,
        freeMode: {
